...
* * **reducing_dataset** - код, который сокращает исходный датасет до такого размера, чтобы осталось по 30000 строк на каждый класс
  * **processing_dataset** - код, который обрабатывает данные сокращённого датасета
  * **finding_model** - код подбора по метрикам оптимального алгоритма для обучения модели
  * **randomforest.py** - код обучения модели алгоритмом Random Forest и сохранение в C файл
  * **preparing_test_data** - код обработки тестовых данных
